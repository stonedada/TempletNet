...
class TransformerUNetParallel(nn.Module):
    def __init__(self, channels: Tuple[int], num_heads: int = 2, image_size: int = 128, is_residual: bool = False, bias = False) -> None:
        ...
        self.encode = nn.ModuleList([EncoderLayer(channels[i], channels[i + 1], is_residual, bias).to('cuda:2') for i in range(len(channels) - 2)])         # transform 6 to 2
        self.bottle_neck = ConvBlock(channels[-2], channels[-1], is_residual, bias).to('cuda:2')                                                            # transform 6 to 2
        self.pos_embed_mhsa = nn.Parameter(torch.randn(1, channels[-1], image_size // (2**(len(channels) - 2)), image_size // (2**(len(channels) - 2)))).to('cuda:2')        # transform 6 to 2
        ...
        self.mhca = nn.ModuleList([MultiHeadCrossAttention(channels[i], num_heads, channels[i], channels[i + 1], bias).to(f'cuda:1') for i in reversed(range(1, len(channels) - 1))])
        self.decode = nn.ModuleList([DecoderLayer(channels[i + 1], channels[i], is_residual, bias).to('cuda:3') for i in reversed(range(1, len(channels) - 1))])     # transform 7 to 3
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(len(self.channels) - 2):
            skip_x, x = self.encode[i](x.to('cuda:2'))                                            # transform 6 to 2
