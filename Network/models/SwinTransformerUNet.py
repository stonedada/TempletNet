...
from timm.models import to_2tuple, DropPath, trunc_normal_
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            x = self.norm(x)
            skip_x = x
        skip_x = skip_x.permuter(0, 2, 1).contiguous().view(B, C, H, W)
        return skip_x, x
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            ...
            skip_x = x
        B, L, C = skip_x.shape
        skip_x = skip_x.permuter(0, 2, 1).contiguous().view(B, C, L ** 0.5, L ** 0.5)
        return skip_x, x
