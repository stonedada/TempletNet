...
from net.data.fnetdataset import FnetDataset
import numpy as np
import torch
...
class BufferedPatchDataset(FnetDataset):
    ...
    def get_random_patch(self):
        ...
        starts = np.array([np.random.randint(0, d - p + 1) if d - p + 1 >= 1 else 0 for d, p in
                           zip(datum[0].size(), self.patch_size)])
